...
## 注意股「連兩天第一款 → 提早做多」訊號探索

> **性質：** 這是**獨立於主策略、但高度相關**的候選訊號探索。所有程式碼都在
> job tmp、**未進 repo**，也**完全沒有動到** `event_backtest.py` /
> `BASELINE_CONFIG` / 既有策略碼。尚未計入滑價／手續費／稅；關鍵價為近似值
> （見下）。未來若要正式使用，需另建可重複執行的程式碼。

### 背景動機與第一款門檻重建

三種**做空**思路（預估處置放空、尾盤判斷放空、市場躲處置
放空），第一步先驗證「第一款門檻能否用每日收盤資料自行重建」。找到官方確切
公式（臺灣證券交易所公布或通知注意交易資訊暨處置作業要點第四條）：

- **條件 A**：最近 6 個營業日累積收盤價漲跌 % **超過 32%**。
- **條件 B**：累積漲跌 % **超過 25%**，且 6 日內起訖兩日收盤價**價差達 50 元**以上。
- 兩者都還需與**全體有價證券平均**及**同類（同產業）平均**的差幅**皆達 20 個
  百分點**以上。

以 2023 年 TWSE 驗證（含還原股價、初次上市無漲跌幅期間、收盤<5 元、本益比
<0 或≥60 不納入同類比較等**四個官方除外情形**）：**約 90% 召回 / 58% 精確**。
殘留誤差診斷為**「全體平均」母體簡化**與 cum6 邊界計算的 1~2pp 小落差（另有
公告日 ±1~2 天偏移佔誤判近四成），**不是門檻或公式錯**。基準日確認為「今日 vs
6 個交易日前收盤」。

### 假說一：「尾盤做價躲處置」——負向結果

用分K檢查 28 個「差一點觸發」的候選（cum6 28-32%、未公告），看是否有人在
收盤前壓價避開門檻：

- **末 30 分鐘成交量佔比：候選組 8.1% vs 對照組 7.9%**，幾乎相同——沒有人為
  操作最該留下的量能痕跡。
- 盤中突破門檻的高點**多在早盤**（開盤衝高後整天緩跌），不是尾盤壓回。

**沒有找到人為操縱的證據。** 但受限於分K只有 1 分鐘顆粒（看不到收盤集合競價
內部的委託動態）與樣本小（單年 28 筆），不能完全否證。

### 假說二：關鍵價放空——負向（方向做反）

在關鍵價放空、隔天收盤回補，2023 年 447 筆：**勝率 26.8%、平均 -8.35%、
總虧損 3,733 萬**。連續兩天命中第一款篩出的是**強勢動能股**，放空等於逆勢
接刀——摸到門檻不是要回頭，是還在噴。

### 核心發現：同一批候選改「做多」

同一批候選改成「第三天摸到關鍵價做多、隔天收盤出場」：

| 年 | 進場數 | 勝率% | 平均% | 中位數% | 總pnl_ntd |
|---|---|---|---|---|---|
| 2020 | 916 | 71.8 | 10.82 | 8.11 | 99,101,524 |
| 2021 | 962 | 75.3 | 9.90 | 7.34 | 95,280,063 |
| 2022（空頭） | 324 | 74.7 | 7.50 | 5.44 | 24,297,615 |
| 2023 | 447 | 73.2 | 8.35 | 6.22 | 37,328,920 |
| 2024 | 738 | 74.4 | 35.02 | 6.38 | 258,461,825 |
| 2025 | 526 | 70.2 | 5.61 | 5.03 | 29,485,625 |
| 全部 | 3,913 | 73.3 | 13.90 | 6.56 | 543,955,573 |

**勝率 70~75% 六年如一（含 2022 空頭年 74.7%），中位數約 5.6~6.6% 穩定。**
注意 **2024 平均 35% 是假象**——被單筆 **+17,324%** 的極端暴衝股撐起，中位數
6.38% 才正常。**看中位數，不要看平均。**

### 執行風險檢查

- **關鍵價貼近漲停的比例 = 0%**（好消息）：關鍵價幾乎不會落在漲停（前收+10%）
  附近，盤中摸得到、不必買在鎖死價。
- 但約 **8.2% 的交易當天全日鎖死漲停**（高=低），實際買不到假設的關鍵價。
  排除後**可執行子集中位數約 5.6%、勝率 72% 依然穩定**。

### 與主策略的重疊

**這批訊號裡約 73~78% 的股票最終真的變成正式處置事件**（day3 前後 [-5,+20]
交易日內有處置 period_start）。代表這本質上是主策略「處置股做多」的**提早進場
版**：提早約 2~4 天（第三天即進，不等處置公告），持有期 1 天 vs 主策略 5~9 天。

### 拆解「有進處置」vs「沒進處置」

| 組別（可執行子集） | 筆數 | 勝率% | 平均% | 中位數% | 總pnl_ntd |
|---|---|---|---|---|---|
| 有進處置（73%） | 2,725 | 77.0 | 8.20 | 6.61 | 223,545,469 |
| 沒進處置（27%） | 869 | 56.0 | 26.12 | 1.46 | 227,012,691 |

- **有進處置的 73% 是穩健核心**：勝率 77%、中位數 6.6%、分布集中。
- **沒進處置的 27% 兩極**：勝率僅 56%、中位數僅 1.46%，正報酬全靠少數極端
  暴衝股（那些 +50%~+17,324% 多落在此組——動能太猛、滾過 6 日窗口反而沒
  累積成連三日進處置）。

**但兩組對總 pnl 的貢獻其實相近**（73% 組約 2.24 億、27% 組約 2.27 億——27%
組筆數少三倍，卻靠單筆爆發力補回來）。因此**不建議額外過濾排除高動能候選**：
維持簡單規則（摸到門檻就進場）即可，過濾反而可能篩掉潛在大贏家。

### 小結

「連兩天第一款 → 提早做多」是一個**穩健、可提早進場、無 look-ahead** 的動能
訊號（純用注意股資料即可篩選）。它的可靠價值主要來自「提早預判會不會進處置」
（與主策略高度重疊），但保留高動能候選能靠右尾補足總報酬。**仍屬探索性質**，
正式使用前需計入交易摩擦、精算關鍵價（納入 20pp 市場差幅）、並建立可重複
執行的程式碼。
