...
的演變、滑價敏感度分析、進場日選擇的決策依據，以及所有已知限制。

最終策略設定與績效摘要見 **[README.md](README.md)**。

**研究期間：2026/07/13 – 2026/07/18**

> **一句話結論**：策略在樂觀滑價假設（0.1%）下有優勢，但在合理保守的滑價
> 假設（1%）下優勢消失。目前尚不足以支持實盤——下一步應實際估算處置股批次
> 撮合的真實滑價，而不是繼續調整進出場參數。
...
- **交易筆數 2,340**：baseline（第 5 天進場）實際產生的交易。差額來自個別
...
> 下表為全進場日掃描（收盤價進場）的整體結果，用於呈現滑價的量級影響。
...
因為進出場各扣一次，而單筆平均只賺約 2%。1% 滑價下策略貼著損益兩平線，
扣掉任何未建模的成本就是負的。**策略的獲利完全依賴「處置股能否以接近收盤價
成交」這個未經實證的假設。**

停損成交模式的影響穩定在約 0.26pp（三種滑價下皆然），因為跳空只佔停損的
18.5% 且幅度不大——這個假設不重要，與前述 close/stop_price 的邏輯矛盾
（值 1.92pp）不是同一量級。
...
中位數遠低於平均，**正的平均值主要靠右尾少數大贏家支撐**（最大 +117%）。
在滑價 0.5% 時中位數就已轉負，代表典型交易已在賠錢，只剩尾部撐住平均。
這種分布對執行品質極度敏感：只要漏接幾筆大贏家（而處置股的大贏家往往正是
最難成交的漲停鎖死標的），實際績效就會遠低於回測。左側尖峰是停損出場群聚
——進場隔日即觸發者，出場價固定落在同一點。
